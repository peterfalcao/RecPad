...
Result = [];
 ...
 for K = Kmin:Kmax
     ...
     SSD_bestK = [];
    for R = 1:Nr
       [Centroides,C_best, SSD] = seq_kmedias(data,K);
        CentroidesK = [CentroidesK; Centroides];
         if ~isfield(C_best,'labels')
             C_best.labels = ones(1,NL);
         end
...
while(1)
    [Centroides,C_best, SSD] = seq_kmedias(data,KoptDB);
    if ~isempty(C_best.labels)
        break;
    end
end
plotSSD(SSD,1,KoptDB);
for i = 1:KoptDB
...
[Centroides,C_best, SSD] = seq_kmedias(data,KoptDI);
for i = 1:KoptDI
    if isempty(find(C_best.labels == i,1))~=1
        Cluster = data(find(C_best.labels == i),:);
        Tamanho(i) = length(find(C_best.labels == i));
