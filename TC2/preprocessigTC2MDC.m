    ...
    for i=1:Nind
        if(i==1)
           ...
           %testcount(i)=counter(i)-trainingcount(i);
            ...
            MtxMDC=[MtxMDC mean(Xtrain,2)];
            ROTmdc=-ones(Nind,1); ROTmdc(i)=1;
            MtxdMDC=[MtxdMDC ROTmdc];
            
    end
    for v=1:length(MtxDtest)
        for j=1:Nind
        NMDC(j)=(norm(Mtxtest(:,v)-MtxMDC(:,j))^2);
        end
        [m,ind]=min(NMDC);
        resNMDC(:,v)=MtxdMDC(:,ind);
        if (isequal(resNMDC(:,v),MtxDtest(:,v)))
...
meanMDC=mean(accuracy);
medianMDC=median(accuracy);
minMDC=min(accuracy);
maxMDC=max(accuracy);
stdMDC=std(accuracy);
ttrain=toc;
